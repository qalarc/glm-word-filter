#!/usr/bin/env python3
"""verify_lexicon_v1.py — bulk-verify vocab/antiscrape_lexicon_v2.json entries
...
SAFETY CONTRACT (do not break):
  ...
  - Per-term verdicts -> results/lexicon_verdicts_v2.jsonl
  ...
  - Counts -> results/logs/verify_v2_counts.json; same json echoed to stdout.
...
WORK = HERE.parent
ENV_FILE = WORK / ".env"
LEXICON_FILE = WORK / "vocab/antiscrape_lexicon_v2.json"
V1_OUT = WORK / "results/lexicon_verdicts_v2.jsonl"
MERGE_OUT = WORK / "results/VERIFIED_BLOCKERS_ALL.json"
COUNTS_OUT = WORK / "results/logs/verify_v2_counts.json"
CHANALYSE = (
    Path.home()
...
def lexicon_items() -> list[tuple[str, str]]:
    """(term, top-level-category) — handles BOTH structures:
    v1 nested {"zh":{sub:[...]}, "en":{...}} and v2 flat {cat:[...]}.
    Dedupe, first wins."""
        ...
        for _subcat, lst in sub.items():
            ...
            for t in lst:
                if isinstance(t, str) and t and t not in seen:
                    seen.add(t)
                    items.append((t, cat))
    # flat top-level categories (v2 structure and any stragglers)
    for cat, lst in lx.items():
        ...
        for t in lst:
            if isinstance(t, str) and t and t not in seen:
                seen.add(t)
                items.append((t, cat if cat in ("zh", "en", "phrases") else "deep"))
    return items
